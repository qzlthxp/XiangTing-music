<template>
  <div class="music-player">
    ...
    <div class="center">
      <span>
        <i title="前一首" class="fa fa-chevron-circle-left fa-2x fa-fw" @click="frontMusic"></i>
      </span>
      <span @click="playMusic">
        <i title="播放" v-show="!isPlayed && !songLoadisShow" class="fa fa-play-circle fa-4x fa-fw"></i>
        <i title="暂停" v-show="isPlayed && !songLoadisShow" class="fa fa-stop-circle fa-4x fa-fw" style="color: var(--active-color)"></i>
        <i title="歌曲正在加载请稍后" v-show="songLoadisShow" style="user-select: none;color: var(--active-color);cursor: default" class="fa fa-spinner fa-pulse fa-4x fa-fw"></i>
      </span>
      <span>
        <i title="后一首" class="fa fa-chevron-circle-right fa-2x fa-fw" @click="nextMusic"></i>
      </span>
    </div>
    <!--播放按钮 End-->
    ...
    <div class="right">
      <!--喜欢 Start-->
      <div class="like">
        <i title="喜欢" v-show="!isLiked" class="fa fa-heart-o fa-lg fa-fw" @click="addLikeSong('right',currentIndex)"></i>
        <i title="取消喜欢" v-show="isLiked" class="fa fa-heart fa-lg activeLike fa-fw" @click="removeLikeSong('right',currentIndex)"></i>
      </div>
      <!--喜欢 End-->
...
<!--      </div>-->
      ...
      <!--选择播放模式 Start-->
      <div class="mode">
        <span v-for="(item,index) in modes"
              :key="index"
      ...
      <div class="list">
        <!--播放列表按钮 Start-->
        <div>
          <i title="播放列表" class="fa fa-bars fa-lg fa-fw" @click="showSongList"></i>
          <div v-show="totalSongList" class="songListNum">{{totalSongList}}</div>
        </div>
        <!--播放列表按钮 End-->
        <!--播放列表详情 Start-->
        <div class="song-list"  v-show="songListIsShow">
          <header>
            ...
            <span @click="CloseSongList"><i class="fa fa-close fa-lg fa-fw"></i></span>
          </header>
          ...
          <div class="song-list-content">
            ...
            <div v-show="totalSongList" class="has-song-list">
              <div class="has-song-list-div"
                   ...
                   :key="index"
              >
                <div class="song-info" @mouseleave="hideSelect">
                  <div class="order">
                    <div v-show="index !== currentIndex">{{index + 1}}</div>
                    <div v-show="index === currentIndex" class="playingAnimation">
                      <span :class="{playingAnimationActive: isPlayed && !songLoadisShow}"></span>
                      <span :class="{playingAnimationActive: isPlayed && !songLoadisShow}"></span>
                      <span :class="{playingAnimationActive: isPlayed && !songLoadisShow}"></span>
                    </div>
                  </div>
                  ...
                  <div v-show="index === currentSelectIndex" class="select">
                    ...
                    <span title="从列表中删除" @click="removeSong(index)"><i class="fa fa-trash-o fa-fw"></i></span>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div class="lock" @click="changeLock">
      <i v-show="isLocked" class="fa fa-lock fa-2x fa-fw"></i>
      <i v-show="!isLocked" class="fa fa-unlock fa-2x fa-fw"></i>
    </div>
    <!--锁定播放器位置 End-->
...
export default {
  ...
  computed: {
    ...
    domSongListDiv() {
      return document.querySelector(`.has-song-list-div:nth-child(${this.currentIndex + 1})`);
    },
    isLiked() {
      return (this.currentIndex !== null) ? this.$store.state.user.songUserLike.some(value => value === this.$store.state.song.songList[this.currentIndex].id) : false;
    },
    listLike() {
      return index => {
        return this.$store.state.user.songUserLike.some(value => value === this.$store.state.song.songList[index].id);
      }
    },
  },
  filters: {
  ...
  methods: {
    ...
    showMusicPlayer(e) {
      ...
      if (!this.isLocked && !this.songListIsShow && !this.showUnlockPlayer) {
        if (e.clientY + this.domMusicPlayer.clientHeight + 50 > document.documentElement.clientHeight) {
          this.domMusicPlayer.style.transform = "translateY(0)";
        }else {
    ...
    playMusic() {
      if (this.totalSongList) {
        let existSrc = this.suffix.some( value => this.$refs.domAudio.src.indexOf(value) > -1);
    ...
    //点击调整播放进度
    getProgress($event) {
      this.currentTime = ($event.offsetX / $event.target.clientWidth) * this.totalTime;
      this.$refs.domAudio.currentTime = this.currentTime;
      this.changeProgress();
    },
    //拖动调整进度
    ProgressDrag($event) {
      ...
      document.onmousemove = ($event) => {
        ...
        this.currentTime = this.$refs.domFill.clientWidth / this.$refs.domProgress.clientWidth * this.totalTime;
      };
    },
    //快速调节音量
     quickChangeVolume() {
       if (!this.volumeSize) {
         ...
         this.$refs.domVolumeFill.style.width = this.volumeSizeHistory * 100 + '%';
    ...
    getVolumeProgress($event) {
      this.$refs.domVolumeFill.style.width = ($event.offsetX / $event.target.clientWidth * 100) + '%';
      this.volumeSize = ($event.offsetX / $event.target.clientWidth).toFixed(1);
      this.$refs.domVolumeBtn.style.left = ($event.offsetX / $event.target.clientWidth * 100) + '%';
    ...
    VolumeProgressDrag($event) {
      ...
      document.onmousemove = $event => {
        ...
        this.volumeSize = this.$refs.domVolumeFill.clientWidth / domVolumeProgressWidth;
        this.volumeSizeHistory = this.volumeSize;
        this.volumeTip = this.$refs.domVolumeFill.clientWidth / domVolumeProgressWidth;
        this.$refs.domVolTip.style.left = this.$refs.domVolumeFill.clientWidth / domVolumeProgressWidth * 100 + '%';
        this.$refs.domAudio.volume = this.volumeSize;
      };
    },
    //获取音量进度提示
    getVolumeTip($event) {
      ...
      this.$refs.domVolTip.style.left = $event.offsetX / $event.target.clientWidth * 100 + '%';
    },
    //获取进度提示
    getProgressTip($event) {
      if (this.totalTime > 0) {
        this.progressTipIsShow = true;
        this.progressTip = ($event.offsetX / $event.target.clientWidth) * this.totalTime;
        this.$refs.domProTip.style.left = ($event.offsetX / $event.target.clientWidth * 100) + '%';
    ...
    //改变进度条
    changeProgress() {
      this.$refs.domFill.style.width = (this.currentTime / this.totalTime * 100) + '%';
      this.$refs.domProgressBtn.style.left = this.$refs.domFill.offsetWidth + 'px';
    },
    //重置数据
    reset() {
      let songListDivs = document.querySelectorAll('.has-song-list-div');
...
.playingAnimationActive{
  animation: playingSpan .5s linear infinite alternate;
}
.song-name{
