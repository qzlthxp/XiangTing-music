        ...
        <div class="select" v-show="currentIndex === index">
          ...
          <span>
            <i
              ...
              @click="addLikeSong(index)"
            ></i>
            <i
              title="取消喜欢"
              v-show="isLiked(index)"
              class="fa fa-heart activeLike fa-fw"
              @click="removeLikeSong(index)"
            ></i>
          </span>
          <span title="加入歌单"
            ><i class="fa fa-plus fa-fw" @click="addThis(index)"></i
          ></span>
        </div>
      </li>
    </ul>
    ...
    isLiked() {
      return (index) => {
        return this.$store.state.user.songUserLike.some(
          (value) => value === this.songInfos[index].id
        )
      }
    },
    songs() {
