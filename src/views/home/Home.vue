  ...
  <div class="home">
    <full-page>
      <template v-slot:pages>
        ...
        <div
          class="page1"
          :class="{ scrollActive: page1ScrollActive }"
          ref="page1"
        >
          <div class="left">
            <div class="desc">
...
export default {
  ...
  mounted() {
    this.getPlayListsByDetail(
      this.selects[this.currentPlayListsIndex].detail_cate_id
    )
    ...
    selects() {
      ...
      if (userInfo.user_id && userInfo.detail_cate) {
        ...
        return [
          {
            name: '为您推荐',
            detail_cate_id: this.$store.state.user.userInfo.detail_cate,
          },
    ...
    async getHotP() {
      try {
        this.playLists = paging(
          quickSortPlayLists((await getHotPlayLists()).data, 'play_number'),
          5
        )
      } catch (e) {
        return e
...
.page1 {
  width: 100%;
  height: 100%;
  padding: 85px var(--default-padding);
  display: flex;
}
.left {
  width: 60%;
...
.left .desc p {
  ...
  top: 2rem;
  opacity: 0;
  animation: 0.5s swift-up cubic-bezier(0.19, 1, 0.22, 1) forwards,
    0.5s fade-in cubic-bezier(0.19, 1, 0.22, 1) forwards;
...
.left .desc h1 span {
  ...
  position: relative;
  top: 2rem;
  animation: 0.3s swift-up cubic-bezier(0.19, 1, 0.22, 1) forwards;
}
.left .desc h1 span i {
  font-style: normal;
  position: relative;
...
.page1.scrollActive .left .page1-banner {
  animation: rightBanner 0.25s cubic-bezier(0.19, 1, 0.22, 1) forwards,
    0.25s fade-in cubic-bezier(0.19, 1, 0.22, 1) forwards;
